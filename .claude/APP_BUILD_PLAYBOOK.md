...
Đã phân tích kỹ (xem `.claude/app-plans/migraine-diary.md`): CloudKit đầy đủ là quá nhiều công sức so với 1 dev solo, cần schema riêng + thư viện RN cộng đồng ít bảo trì. **Pattern đã chọn và nên tái dùng**: JS bridge tới ubiquity container qua native module nhỏ (`getContainerURL`), lưu file JSON snapshot theo device-id, merge record-level "ai `updatedAt` mới hơn thắng" khi sync. Xem `src/app/services/iCloudSync.ts` làm mẫu. Luôn có fallback graceful khi native module chưa được wire (`isAvailable()` trả `false`, không throw).
